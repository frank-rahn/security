	...
	public void run() {
		...
		try {
			...
			appendDesc("Message Digest Info")
				...
				.appendToString(digest_1).appendln();
			...
			appendDesc("Message Digest Info")
				...
				.appendToString(digest_512).appendln();
			...
			appendDesc("Kryptologische Hashfunktion SHA-1", 1)
				.appendValue("Text", TEXT).appendValue("Hash (Raw)", out)
				.appendValue("Hash (Base64)", new String(encodeBase64(out)))
				.appendValue("Hash (Length)", out.length).appendln();
