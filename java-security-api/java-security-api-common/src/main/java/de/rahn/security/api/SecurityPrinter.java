...
/**
 * Diese Klasse soll Objekte der Java-Security-API bzw. der Java Cryptography Extension (JCE)
 * ausgeben.
 *
 * @author Frank W. Rahn
 */
public abstract class SecurityPrinter implements Appendable, Closeable, Flushable, Runnable {
  ...
  public SecurityPrinter appendDesc(String title) {
    String n = " #" + number++ + "  " + (currentTimeMillis() - startThread) + " ms";
  ...
  public SecurityPrinter appendValue(String name, Object value) {
    writer.append(rightPad(name, width)).append(" = ").append(String.valueOf(value)).println();
    return this;
  }
   ...
   * @return diesen Printer
   */
  public <T> SecurityPrinter appendValue(String name, Enumeration<T> enumeration) {
    writer.append(rightPad(name, width)).append(" = ");
    ...
    if (enumeration != null) {
      ...
      while (enumeration.hasMoreElements()) {
        ...
        writer.append(String.valueOf(enumeration.nextElement().toString()));
        notFirst = true;
      }
      writer.append(']');
    } else {
